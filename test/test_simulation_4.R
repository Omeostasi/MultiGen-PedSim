...
# Filenames
file_description <- sprintf("_Ne_%g_segSites_%g_minSnpFreq_%g_nFounders_%g_nGenerations_%g", Ne, segSites, minSnpFreq, nFounders_final, nGenerations_pop)

ped_file   <- sprintf("sim_pedigree_run%s.csv", file_description)
mom_file   <- sprintf("sim_mom_phenotypes_run%s.csv", file_description)
kid_file   <- sprintf("sim_child_phenotypes_run%s.csv", file_description)
plink_base <- sprintf("sim_genotyped_subset_run%s", file_description)
...
## Write PLINK in chunks to stay within writePlink()'s matrix size limit.
## Two known bugs in writePlink() are worked around here:
##   1. Parent IDs are always written as 0 — patched via getPed(pop_geno) during streaming.
##   2. The last line of each chunk .ped is truncated — fixed by appending a dummy
##      individual (first of the next chunk) and stripping it after writing.
