...
##  IT HAS BEEN BUILT THINKING ABOUT 5 GENERATIONS, DESCRIBED AS:
...
##   
##  IF THE POPULATION HAS BEEN ALREADY GENERATED (AND IT IS IN THE RESULTS FOLDER), THE SCRIPT RECOMPUTES ONLY THE PHENOTYPES AND GENETIC COMPONENTS.
...
##   4)  prevASC_child -> population prevalence of ASC in children (both sexes combined)
##   5)  ASC_male_ratio -> male-to-female ratio for ASC diagnosis
##   6)  betaPreg      -> effect of pregnancy CMC exposure on child ASC liability (liability scale)
...
##   FOR DIFFERENT POPULATIONS, THE POPULATION PARAMETERS MUST BE CHANGED MANUALLY (e.g., nGenerations_pop, per-generations parametrs).
##
##   CMC CAN BE ANY MOTHER TRAIT INFLUENCING THE PHENOTYPE OF THE CHILDREN.
##   ASC CAN BE ANY CHILDREN TRAIT INVESTIGATED.
##   THESIS EXPLAINS THE REASONS BEHIND THE NAME CONVENTION.
## -------------------------------------------------------------------------------------------------------------------------------------------------------------
...
prevASC_child  <- as.numeric(if (length(args) >= 4) args[4] else 0.02)  # population prevalence of ASC in children (both sexes combined)
...
n_geno_ASC    <- 25000    # target number of ASC cases to genotype
n_geno_random <- 125000   # target number of random non-ASC individuals to genotype
## ---------------------
