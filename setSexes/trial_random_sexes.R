...
nFounder   <- 800
nChr       <- 22
segSites   <- 400       # segregating sites per chromosome
nSnpPerChr <- 200       # SNPs per chromosome on chip (observed)

## Population / study design
nParents   <- 2000      # size of adult generation
nMothers   <- 700
...
sex_table <- data.frame(
  id  = children@id,
  sex = children@sex,   # "M" or "F"
  mother_id = child_moms
)

result <- do.call(rbind, lapply(split(sex_table, sex_table$mother_id), function(df) {
  data.frame(
    mother_id  = df$mother_id[1],
    n_children = nrow(df),
    n_M        = sum(df$sex == "M"),
    n_F        = sum(df$sex == "F"),
    n_M_ratio = sum(df$sex == "M") / nrow(df)
  )
}))

result <- result[order(result$mother_id), ]
rownames(result) <- NULL


print(result)

saveRDS(result, file = "pop_analysis/results_m_f_random_sexes.rds")
