...
Ne <- as.integer(args[1])

if (Ne == 125000) {
  Ne_range <- seq(1 * Ne, 50 * Ne, by = 25000)
} else if (Ne == 2500) {
  Ne_range <- seq(1 * Ne, 50 * Ne, by = 5000)
} else {
  Ne_range <- seq(1 * Ne, 50 * Ne, by = 50)
}

cat("Running with", Ne_range, "as Ne values\n")
...
# segSites (Number of Segregating Sites)
  ...
  # threads ranges
  # Read threads from SLURM environment variable, fallback to 1
  n_threads <- as.integer(Sys.getenv("SLURM_CPUS_PER_TASK", unset = "1"))
  cat("Running with", n_threads, "threads\n")
  
  # threads_range now comes from SLURM
  ...
  saveRDS(comparison_by_param, file = sprintf("results/bench_Ne_extended_%d.rds", Ne))
